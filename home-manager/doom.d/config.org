...
** tree-sitterグラマーの自動インストール
=typescript-ts-mode= / =tsx-ts-mode=（ネイティブtreesit）を使うが、グラマー本体
（=.dylib=）はnixにもDoomの宣言的パッケージ管理（=packages.el=）にも乗らず、
=~/.config/emacs/.local/etc/tree-sitter/= に手動ビルドされた状態でしか存在しない。
=doom purge= や新規マシン展開でこのディレクトリが消えると
"Cannot activate tree-sitter" 警告が復活するので、起動時に無ければ自動で
ビルドし直す。
#+begin_src emacs-lisp
